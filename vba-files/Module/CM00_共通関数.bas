...
' |
' | @Program naming rule
' |  ProgramLblID: xx.99.99-xxxxxxxxx-vx.y.z-yyyymmdd.suffix   x.y.z≡version number
' |              |--xx(SystemSymbl)
' |              |----99(Subsystem#)
' |              |------.(priod≡separator)
' |              |-------99(Module#)
...
' モジュールレベル変数 ・・・ モジュール内でのみ使用可能　モジュールの先頭（最初のSubまたはFunctionより前）に書く
...
' |     │ (99.zStuidoLibrary)
' |     ├ nn.システム名　 SysPath（フルパス）\　nn.n.SysName（フォルダ名）
' |     │      │ (99.2.新住所録管理(zz2))
' |     │      ├ nn.n.サブシステム名   SubSysPath（フルパス）　SubSysName（フォルダ名） （親ディレクトリ≡親フォルダ　../　ParentFolder)
' |     │      │      │
' |     │      │      ├ @README-新住所録の運用Jobの整備
' |     │      │      │     │
' |     │      │      │     ├ !Repository(開発環境)
' |     │      │      │     ├ !運用の手引き
' |     │      │      │
' |     │      │      ├ 1.マスター群
' |     │      │      │
' |             │      ├ 2.Job-ジョブ名  folderPath（フルパス） folderName（フォルダ名） (カレントディレクトリ≡基点フォルダ　　./　)
' |                     │     │
' |   コピペ用記号集    │     ├ 2.1.inputData
' |-----------------    │     ├ 2.2.proc-実行プログラム名(zz2)
' |     ┣ ┠ ┝ ├     │     │   │
...
' --------------------------------------+----------------------------------------
' |  命名規則の統一
' |     Public変数  先頭を大文字    ≡ pascalCase    …… Excel VBAシステムに関する変数--→ 共通関数で定義
' |                                                …… プログラムで共通の変数--→ m0_メインモジュールで定義
...
' ファイル名書式：LblIDey\SysID.xx.xx_programName-vX.Y.Z_yyyymmdd.sufix
...
'    Set fso = CreateObject("Scripting.FileSystemObject")
    ...
    prgFileName = myBook
' ProgramLblID: xx99.99-xxxxxxxxx-vx.y.z-yyyymmdd.suffix   x.y.z≡version number を分割
'             |--xx(SystemSymbl)
'             |----99(Subsystem#)
'             |------.(priod≡separator)
'             |-------99(Module#)
...
'   Call R_DocInfoGet           ' ファイルのパス情報（DocLblIDblKeyProgramID等を得る
